...
res = sb.table('dialogs').select('id, transcript').order('created_at', desc=True).limit(1).execute()
if res.data:
    print(f"Latest Dialog ID: {res.data[0]['id']}")
    for t in res.data[0]['transcript']:
