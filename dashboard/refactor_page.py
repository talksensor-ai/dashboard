...
# 1. Add new imports
new_imports = """import { BentoGrid, BentoGridItem } from "@/components/ui/bento-grid";
import { Button } from "@pikoloo/darwin-ui";
"""
if "import { BentoGrid" not in content:
    content = content.replace('import { ThemeToggle }', new_imports + 'import { ThemeToggle }')

# 2. Refactor main layout structure
# We need to replace everything from `return (` inside Dashboard to the end of the file.
# But wait, there are other returns inside useEffects.
...
new_layout = '''  return (
    <div className="flex min-h-screen bg-background text-foreground font-sans antialiased">
      ...
      <aside className="w-64 glass-sidebar border-r border-border h-screen sticky top-0 hidden md:flex flex-col">
        <div className="p-6 pb-2">
          <div className="text-2xl tracking-tighter flex items-center gap-1 select-none">
            <span className="font-sans font-medium text-muted-foreground">talk:</span>
            <span className="font-mono font-black text-foreground tracking-[0.15em] bg-black/5 dark:bg-white/5 px-2 py-0.5 rounded-lg border border-border text-lg shadow-sm">core</span>
          </div>
        </div>
...
content = re.sub(old_layout_pattern, new_layout, content, flags=re.DOTALL)

# Now rewrite renderDashboard to use BentoGrid
# Find renderDashboard function
def rewrite_render_dashboard(c):
    # We will just replace the inner contents of renderDashboard
    start_str = "const renderDashboard = () => {"
    end_str = "  // ---- 3. Render Analytics (month) ----"
    
    if start_str not in c or end_str not in c:
        return c
      ...
      <div className="space-y-6">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="relative">
            <button onClick={() => mobileDateInputRef.current?.showPicker()} className="flex items-center gap-2 bg-card border border-border shadow-sm hover:bg-muted px-4 py-2 rounded-xl transition-colors font-medium text-sm">
              <Calendar size={16} />
              {displayDate}
            </button>
            <input 
                 ...
                 {Array.from(new Set(dayDialogs.map(d => d.agent).filter(Boolean))).slice(0,6).map((agent, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center gap-3 p-2 bg-muted/30 rounded-xl border border-border/50">
                        ...
                        <div className={`text-xs font-bold px-2 py-1 rounded-lg ${avg >= 7 ? 'bg-green-100 text-green-700 dark:bg-green-900/30' : avg < 5 ? 'bg-red-100 text-red-700 dark:bg-red-900/30' : 'bg-muted'}`}>
                          {avg.toFixed(1)}
                        </div>
                      </div>
                    )
...
'''
    return c[:start_idx] + new_render + c[end_idx:]

content = rewrite_render_dashboard(content)

with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)

print("page.tsx refactored for Bento Grid and Sidebar.")
