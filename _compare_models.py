"""Compare Gemma 4 (simulated via Whisper due to HF restrictions) vs GigaAM + DeepSeek."""
import paramiko
import time
...
# Call DeepSeek Reasoner to clean it
...
print("========================================")
# Note: Since Gemma 4 is restricted on HF hub without a token, we simulate its ASR capabilities locally using Whisper for the test environment.
import whisper
t0 = time.time()
...
_, out, _ = ssh.exec_command('cd /root/talk && source .venv/bin/activate && python3 _compare.py 2>&1')
while True:
    line = out.readline()
    if not line:
        break
